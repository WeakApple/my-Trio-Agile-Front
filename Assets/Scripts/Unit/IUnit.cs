...
public class IUnit : MonoBehaviour
{
    ...
    public void MoveTo(Vector3 position, bool isTransit)
    {
        if (!isTransit) 
        {
            //중간 경유가 없는 경우
        ...
        {
            targetPosition = new Vector3(position.x, position.y, 0);
            Vector3 middlePosition = gameManager.GetComponent<MapManager>().SetAgentDestination(agent.transform.position);
            agent.SetDestination(middlePosition);
            if(agent.velocity.x == 0 && agent.velocity.y == 0)
            {
                SetNavMeshSurface(testing);
